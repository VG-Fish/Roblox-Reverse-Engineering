// bundle: page___4ddb014d81e32d9267e3fba40c01c349_m
  ...
  Roblox.GenericConfirmation = (function () {
    ...
    function open(properties) {
      ...
      if (properties.allowHtmlContentInBody)
        $("[data-modal-handle='confirmation'] div.Message").html(
          properties.bodyContent,
        );
      else
        $("[data-modal-handle='confirmation'] div.Message").text(
          properties.bodyContent,
        );
      ...
      if (properties.allowHtmlContentInFooter)
        $(
          '[data-modal-handle="confirmation"] div.ConfirmationModalFooter',
        ).html(properties.footerText);
      else
        $(
          '[data-modal-handle="confirmation"] div.ConfirmationModalFooter',
        ).text(properties.footerText);
    ...
    function enableButtons() {
      ...
      var noBtn = $(BUTTON_SELECTOR_NO);
      var classesToRemove =
        BUTTON_CLASS_BLUE_DISABLED +
        " " +
        BUTTON_CLASS_GRAY_DISABLED +
        " " +
        BUTTON_CLASS_GREEN_DISABLED;
...
Roblox.AbuseReportDispatcher = (function () {
  ...
  var triggerUrlAction = function (url) {
    ...
    if (action) {
      ...
      if (getParamFromQueryString("conversationid", url)) {
        payload["conversationId"] = getParamFromQueryString(
          "conversationid",
          url,
        );
      }
      ...
      if (payload.id && payload.redirectUrl) {
        ...
        var embedUrlPath =
          "abusereport/embedded/" + action + "?" + newQuerystring;
        var embeddedUrl = Roblox.Endpoints.getAbsoluteUrl("/" + embedUrlPath);
        if (pvMeta.inApp) {
          if (pvMeta.inAppEnabled) {
            var params = { urlPath: embedUrlPath, feature: "Abuse Report" };
  ...
  return { triggerUrlAction: triggerUrlAction, initialize: initialize };
})();
