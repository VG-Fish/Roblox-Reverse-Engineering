...
Roblox.Performance = (function () {
  ...
  function isUiPerformanceTrackingInDiagEnabled() {
    ...
    var performanceRelativeValue = uiPerformanceElement.data(
      "ui-performance-relative-value",
    );
    return Math.random() < performanceRelativeValue && isPerformanceLibValid();
  }
  function isPerformanceEnabled() {
    var performanceRelativeValue = performanceElement.data(
      "performance-relative-value",
    );
    return Math.random() < performanceRelativeValue && isPerformanceLibValid();
  }
  function isEventStreamEnabled() {
    var eventStreamPercentage = performanceElement.data(
      "send-event-percentage",
    );
    return (
      Roblox.EventStream &&
      eventStreamPercentage &&
      eventStreamPercentage != 0 &&
      Math.random() < eventStreamPercentage &&
  ...
  function getPageLoadMeasure() {
    if (timing && timing.domComplete && timing.navigationStart) {
      ...
      if (duration > 0) {
        return { name: "PageLoad", duration: duration };
      }
    }
    return null;
  }
  function getFirstByteMeasure() {
    if (timing && timing.responseEnd && timing.navigationStart) {
      ...
      if (duration > 0 && duration.toString().length < 6) {
        return { name: "FirstByte", duration: duration };
      }
    }
    return null;
  }
